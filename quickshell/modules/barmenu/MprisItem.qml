...
Rectangle {
  ...
  clip: true
  color: Colors.current.highlightLow
  radius: 20
  ...
  Timer {
    ...
    interval: 500
    repeat: true
    running: Globals.barState == "FULLY_EXPANDED" && Globals.swipeIndex == 3 && rect.player.isPlaying
  ...
  Timer {
    ...
    interval: 500
    repeat: true
    running: Globals.barState != "COLLAPSED" && rect.player.isPlaying

    onRunningChanged: {
      Globals.mprisDotRotation += (mprisDotRotateTimer.running) ? 6 : 0;
    }
    onTriggered: Globals.mprisDotRotation += 6
  }
